"""
E-Com67 Platform Admin Dashboard CI/CD Pipeline Stack

This stack creates a CI/CD pipeline using AWS CodePipeline to automatically
build and deploy the admin dashboard React application when changes are pushed
to the master branch.

Architecture:
    Source (CodeCommit) -> Build (npm build) -> Deploy (S3 + CloudFront invalidation)

The pipeline monitors the frontends/admin-dashboard/ directory for changes.
"""
...
class AdminPipelineStack(Stack):
    """
    CI/CD Pipeline stack for E-Com67 Admin Dashboard Frontend.
    ...
    The pipeline automatically triggers on changes to frontends/admin-dashboard/
    """
    ...
    def __init__(
        ...
        branch: str = "master",
        admin_bucket: s3.IBucket = None,
        admin_distribution: cloudfront.IDistribution = None,
        **kwargs
    ) -> None:
        ...
        build_project = codebuild.PipelineProject(
            self,
            "AdminBuildProject",
            project_name="e-com67-admin-dashboard-build",
            build_spec=codebuild.BuildSpec.from_object({
                ...
                "artifacts": {
                    "base-directory": "frontends/admin-dashboard/dist",
                    "files": ["**/*"]
                },
                "cache": {
                    "paths": [
                        "frontends/admin-dashboard/node_modules/**/*"
                    ]
                }
            }),
        ...
        invalidation_project = codebuild.PipelineProject(
            self,
            "AdminInvalidationProject",
            project_name="e-com67-admin-dashboard-invalidation",
            build_spec=codebuild.BuildSpec.from_object({
                "version": "0.2",
                "phases": {
                    "build": {
        ...
        invalidation_project.add_to_role_policy(
            iam.PolicyStatement(
                ...
                resources=[f"arn:aws:cloudfront::{self.account}:distribution/{admin_distribution.distribution_id}"]
            )
        )
        ...
        pipeline = codepipeline.Pipeline(
            self,
            "AdminPipeline",
            pipeline_name="e-com67-admin-dashboard-pipeline",
            stages=[
        ...
        CfnOutput(
            ...
            description="Name of the admin dashboard CI/CD pipeline"
        )
        ...
        CfnOutput(
            ...
            description="ARN of the admin dashboard CI/CD pipeline"
        )
